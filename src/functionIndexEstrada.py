...
for ppi in ppis:
	...
	for geneRow in genesFile:
		...
		if len(functionGene1) > 0 and len(functionGene2) > 0:
			...
			total1 = list(set(functionGene1[0]))
			total2 = list(set(functionGene2[0]))
			index = (float(2) * float(float(len(overlap))) / float(len(total1) + len(total2)))
			overlapFile.write(ppi[:-2] + ';' + str(index) + ';' + str(len(overlap)) + ';' + str(float(len(total1) + len(total2))) +'\n')
			break
